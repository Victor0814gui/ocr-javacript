...
const diretorio = '../../texts';

const respostas = {};
let totalFormularios = 0;
let totalHomens = 0;
let totalMulheres = 0;
const esperado = 124; // Total esperado

const arquivosNaoDiscrepantes = [];
...
fs.readdir(diretorio, (err, files) => {
  ...
  files.forEach(file => {
    ...
    try {
      ...
      if (totalRespostas === esperado) {
        arquivosNaoDiscrepantes.push(file);
      }
    } catch (error) {
  ...
  // Exibir arquivos não discrepantes
  console.log('Arquivos não discrepantes:');
  arquivosNaoDiscrepantes.forEach(arquivo => {
    console.log(arquivo);
  });
});
